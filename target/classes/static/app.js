...
async function loadResultats() {
    try {
        ...
        for (const [key, candData] of Object.entries(groupedByCandidate)) {
            let resultsHtml = '';
            let totalDiff = 0;
            let isGlobalAdmis = true;

            candData.results.forEach(r => {
                totalDiff += r.reliquat;
                if (r.status === 'Ajourné') isGlobalAdmis = false;
                    ...
                    <div class="result-item">
                        ...
                        <div class="result-score-status" style="display: flex; flex-direction: column; align-items: flex-end; gap: 0.2rem;">
                            <div style="display: flex; align-items: center; gap: 0.5rem;">
                                <span class="result-score" style="font-size: 1.1rem; font-weight: bold;">${r.noteCalculee.toFixed(2)}</span>
                                <span class="result-reliquat" style="font-size: 0.8rem; color: ${r.reliquat >= 0 ? '#4ade80' : '#f87171'};">
                                    (${r.reliquat >= 0 ? '+' : ''}${r.reliquat.toFixed(2)})
                                </span>
                            </div>
                            <span class="result-status ${statusClass}" style="font-size: 0.7rem;">${r.status}</span>
                        </div>
                    </div>
                `;
            });

            const globalStatusClass = isGlobalAdmis ? 'status-admis' : 'status-ajourne';
            const globalStatusText = isGlobalAdmis ? 'ADMIS' : 'AJOURNÉ';
            ...
            container.innerHTML += `
                <div class="result-card">
                    ...
                    <div class="result-card-footer" style="padding: 1.5rem; border-top: 1px solid rgba(255,255,255,0.1); display: flex; flex-direction: column; gap: 1rem; align-items: center;">
                        <div class="total-score-row" style="display: flex; justify-content: space-between; width: 100%; font-weight: 600;">
                            <span>Reliquat Global :</span>
                            <span class="total-val ${totalDiff >= 0 ? 'status-admis' : 'status-ajourne'}" style="font-size: 1.2rem;">${totalDiff >= 0 ? '+' : ''}${totalDiff.toFixed(2)}</span>
                        </div>
                        <div class="global-status-badge ${globalStatusClass}" style="width: 100%; text-align: center; padding: 0.8rem; border-radius: 8px; font-weight: bold; font-size: 1.1rem; letter-spacing: 1px;">
                            ${globalStatusText}
                        </div>
                    </div>
                </div>
